...
interface AppContextType {
  ...
  toolCredits: number;
  deductToolCredit: (amount: number) => void;
  isProUser: boolean;
  planTier: string;
...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [toolCredits, setToolCredits] = useState<number>(15);
  const deductToolCredit = (amount: number) => {
    setToolCredits((prev) => Math.max(0, prev - amount));
  };
  const isProUser = Boolean(userAccount?.isPremium || (userAccount?.membershipPlan && userAccount.membershipPlan !== 'free'));
  const planTier = userAccount?.planTier || userAccount?.membershipPlan || 'free';
  const promptRequestsRemaining = userAccount?.promptRequestsAllowed ?? (planTier === 'vip' ? 5 : planTier === 'pro' ? 2 : planTier === 'starter' ? 1 : 0);
  const [isProCheckoutModalOpen, setIsProCheckoutModalOpen] = useState(false);
  ...
  const updatePromptRequestStatus = async (id: string, status: 'pending' | 'in_progress' | 'completed' | 'rejected', fulfilledPrompt?: string): Promise<void> => {
    setPromptRequests((prev) =>
      prev.map((r) => (r.id === id ? { ...r, status, fulfilledPrompt: fulfilledPrompt ?? r.fulfilledPrompt } : r))
    );
    ...
    try {
      const res = await fetch('/api/prompt-requests', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id, status, fulfilledPrompt }),
      });
      const data = await res.json();
      if (data.success && Array.isArray(data.data)) {
        setPromptRequests(data.data);
...
export const useApp = () => {
  ...
  if (!context) {
    return {
      ...
      categories: [],
      tags: [],
      settings: { siteName: 'Trending Gemini Prompts', siteTagline: '', siteUrl: '', logoText: 'Prompts', heroHeadline: '', heroSubheadline: '', defaultTool: 'Midjourney', enableAiGenerator: true, footerText: '', adminEmail: '' },
      bookmarkedIds: [],
      likedIds: [],
      searchQuery: '',
      setSearchQuery: () => {},
...
export const useBookmarks = () => {
  const { bookmarkedIds, toggleBookmark, likedIds, toggleLike } = useApp();
  return { bookmarkedIds, toggleBookmark, likedIds, toggleLike };
};

export const useTasteProfile = () => {
  const { tasteProfile, updateTasteProfile, isTasteModalOpen, setIsTasteModalOpen, recordPromptClick } = useApp();
  return { tasteProfile, updateTasteProfile, isTasteModalOpen, setIsTasteModalOpen, recordPromptClick };
};
