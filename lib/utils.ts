...
export function getPromptMetaDescription(post: { promptText?: string; title?: string }): string {
  if (post.promptText) return post.promptText.slice(0, 160);
  return `Check out this AI photo prompt: ${post.title || 'Prompt'}`;
}
