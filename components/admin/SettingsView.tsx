...
export const SettingsView = () => {
  const { settings, saveSettings, posts, categories, tags, restorePromptCards, refreshData, showToast, setAdminSubView } = useApp();
  ...
  // Multiplier & Minimum Floor Tool State
  ...
  const handleApplyMultiplier = async () => {
    if (!posts || posts.length === 0) {
      showToast('No prompt posts found to multiply.');
      return;
    }
    if (!confirm(`Are you sure you want to multiply all views and likes by ${multiplierFactor}x for all ${posts.length} prompts? This is saved permanently to the database.`)) {
      return;
    }
    ...
    try {
      const updatedPosts = posts.map((p) => ({
        ...p,
        viewsCount: Math.round((p.viewsCount || 0) * multiplierFactor),
        likesCount: Math.round((p.likesCount || 0) * multiplierFactor),
      }));

      await restorePromptCards(updatedPosts, 'replace', categories, tags);
      showToast(`Successfully multiplied views and likes by ${multiplierFactor}x for ${updatedPosts.length} prompts!`);
    } catch (err: any) {
      showToast(`Failed to apply multiplier: ${err.message || 'Unknown error'}`);
    } finally {
      setIsApplyingMultiplier(false);
    }
  };

  const handleApplyMinimumFloor = async () => {
    if (!posts || posts.length === 0) {
      showToast('No prompt posts found.');
      return;
    }
    if (!confirm(`Apply minimum floors (Views ≥ ${minViewsFloor}, Likes ≥ ${minLikesFloor}) to all prompts? Prompts with lower counts will be bumped up.`)) {
      return;
    }

    setIsApplyingFloor(true);
    try {
      let countBoosted = 0;
      const updatedPosts = posts.map((p) => {
        let changed = false;
        let v = p.viewsCount || 0;
        let l = p.likesCount || 0;
        if (v < minViewsFloor) {
          v = minViewsFloor;
          changed = true;
        }
      ...
      await restorePromptCards(updatedPosts, 'replace', categories, tags);
      showToast(`Successfully applied minimum floors! ${countBoosted} prompt(s) boosted.`);
    } catch (err: any) {
  ...
  return (
    <div className="p-6 sm:p-8 space-y-6 max-w-5xl mx-auto">
      ...
      {/* Views & Likes Multiplier & Minimum Floor Tool */}
      <div className="bg-white dark:bg-neutral-900 p-6 rounded-3xl border border-neutral-200 dark:border-neutral-800 shadow-sm space-y-6">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-bold text-neutral-900 dark:text-white flex items-center gap-2">
            ...
            <span>Views & Likes Multiplier & Minimum Floor Tool</span>
          </h3>
          <span className="text-[10px] font-bold text-amber-600 bg-amber-50 dark:bg-amber-950/60 px-2.5 py-0.5 rounded-full">
            Permanent DB Update
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
          ...
          <div className="p-4 rounded-2xl bg-neutral-50 dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 space-y-4">
            <h4 className="text-xs font-bold text-neutral-900 dark:text-white uppercase tracking-wider">
              Views & Likes Multiplier
            </h4>
            <div className="flex items-center gap-3">
              <div className="flex-1">
                <label className="block text-[11px] font-semibold text-neutral-500 mb-1">Multiplier Factor</label>
                <select
                ...
                >
                  ...
                  <option value={10}>10x (10 Times)</option>
            ...
            <button
              ...
              disabled={isApplyingMultiplier}
              className="w-full py-2.5 px-4 rounded-xl bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold shadow-md shadow-amber-600/30 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
            >
              ...
              <span>Multiply All Views & Likes ({multiplierFactor}x)</span>
            </button>
          </div>

          {/* Minimum Floor Threshold Section */}
          <div className="p-4 rounded-2xl bg-neutral-50 dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 space-y-4">
            <h4 className="text-xs font-bold text-neutral-900 dark:text-white uppercase tracking-wider">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-[11px] font-semibold text-neutral-500 mb-1">Min Views Floor</label>
                <input
                  type="number"
                  value={minViewsFloor}
                  onChange={(e) => setMinViewsFloor(Number(e.target.value))}
                  className="w-full px-3 py-2 rounded-xl bg-white dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-700 text-neutral-900 dark:text-white text-xs font-bold focus:outline-none focus:ring-2 focus:ring-blue-500"
                  min={1}
                />
              </div>
              <div>
                <label className="block text-[11px] font-semibold text-neutral-500 mb-1">Min Likes Floor</label>
                <input
                  type="number"
                  value={minLikesFloor}
                  onChange={(e) => setMinLikesFloor(Number(e.target.value))}
                  className="w-full px-3 py-2 rounded-xl bg-white dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-700 text-neutral-900 dark:text-white text-xs font-bold focus:outline-none focus:ring-2 focus:ring-blue-500"
                  min={1}
                />
              </div>
            ...
            <button
              type="button"
              onClick={handleApplyMinimumFloor}
              disabled={isApplyingFloor}
              className="w-full py-2.5 px-4 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold shadow-md shadow-blue-600/30 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
            >
              {isApplyingFloor ? (
                <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
              ) : (
