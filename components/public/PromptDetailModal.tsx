...
export const PromptDetailModal = () => {
  ...
  // Keep historyStack synchronized with selectedPost using useEffect instead of render-time state updates
  useEffect(() => {
    if (!selectedPost) {
      setHistoryStack([]);
    ...
    if (selectedPost.id !== prevSelectedId) {
      setPrevSelectedId(selectedPost.id);
      setHistoryStack((prev) => {
        const idx = prev.findIndex((p) => p.id === selectedPost.id);
        if (idx !== -1) {
  ...
  }, [selectedPost, prevSelectedId]);

  const isLiked = selectedPost ? likedIds?.includes(selectedPost.id) : false;
  const currentPost = posts.find((p: any) => p.id === selectedPost?.id) || selectedPost;
  ...
  useEffect(() => {
    if (selectedPost?.id) {
      ...
      if (typeof window !== 'undefined' && !sessionStorage.getItem(sessionKey)) {
        ...
        setPosts((prev: any) =>
          prev.map((p: any) => (p.id === postId ? { ...p, viewsCount: (p.viewsCount || 0) + 1 } : p))
        );
      }
    }
  ...
  const allRecommendedPins = useMemo(() => {
    ...
    const otherPublished = posts.filter((p: any) => {
    ...
    const targetTags = new Set((selectedPost.tags || []).map((t: string) => t.toLowerCase()));
    const targetCategory = selectedPost.category?.toLowerCase();

    const scored = otherPublished.map((post: any) => {
      let score = 0;
    ...
    const relevantList = scored.map((item: any) => item.post);

    // Combine relevant items and deduplicate strictly
    const combined: PromptPost[] = [];

    relevantList.forEach((p: any) => {
      if (
        !seenIds.has(p.id) &&
