      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-3 sm:gap-4">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          <button
            ...
            className={`p-2 sm:px-3 sm:py-2 rounded-full text-xs font-bold transition-all flex items-center gap-1.5 ${
              currentView === 'admin'
                ? 'bg-neutral-900 text-white dark:bg-white dark:text-neutral-950 ring-2 ring-[#E60023]/30'
                : isAuthenticated
                ? 'bg-amber-500/10 hover:bg-amber-500/20 text-amber-700 dark:text-amber-300 border border-amber-500/30'
                : 'bg-[#efefef] dark:bg-neutral-800 hover:bg-[#e2e2e2] dark:hover:bg-neutral-700 text-neutral-800 dark:text-neutral-200'
            }`}
            title={isAuthenticated ? 'Open Admin Control Panel' : 'Admin Panel Login'}
            id="header-admin-btn"
          >
            <ShieldCheck className={`w-4 h-4 ${isAuthenticated ? 'text-amber-600 dark:text-amber-400' : 'text-neutral-600 dark:text-neutral-400'}`} />
