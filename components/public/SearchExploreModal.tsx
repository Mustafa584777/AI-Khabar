...
export const SearchExploreModal = () => {
  ...
  const mostViewedCategories = useMemo(() => {
    ...
    const catWithStats = categories.map((cat: any) => {
      const catPosts = publishedPosts.filter(
        (p: any) => p.category?.toLowerCase() === cat.name?.toLowerCase()
      );

      // Sort by viewsCount desc
      const sortedPosts = [...catPosts].sort((a: any, b: any) => (b.viewsCount || 0) - (a.viewsCount || 0));
      const totalViews = catPosts.reduce((sum: number, p: any) => sum + (p.viewsCount || 0), 0);
    ...
    return catWithStats.sort((a: any, b: any) => b.totalViews - a.totalViews || b.postCount - a.postCount);
  }, [categories, posts]);
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-7">
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {popularSearchQueries.map((query: string) => (
                <button
                  key={query}
                  type="button"
          ...
          <div className="space-y-4 pt-2">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {mostViewedCategories.map((cat: any) => (
                <div
                  key={cat.id}
