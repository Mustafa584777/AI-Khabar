...
Return the final response strictly conforming to the required JSON schema.`;
    ...
    if (action === 'text_to_prompt') {
      ...
      if (!apiKey) {
        const fallbackPrompt = `Photorealistic cinematic photography of ${idea}, featuring ${lighting || 'dramatic volumetric cinematic lighting'} and ${colorGrading || 'teal and orange cinematic color grading'}, character: ${gender || 'cinematic subject'}, shot on 35mm lens, f/1.8 aperture, 8k resolution, photorealistic details --ar 16:9 --v 6.1 --style raw`;
        return NextResponse.json({
          success: true,
          data: {
            title: idea.slice(0, 40),
            summary: `Detailed AI prompt crafted from idea: "${idea}"`,
            promptText: fallbackPrompt,
            negativePrompt: 'blurry, low resolution, distorted anatomy, bad proportions, watermark, ugly',
            camera: 'Hasselblad 50mm f/1.2 lens, studio lighting',
            lighting: lighting || 'Cinematic Golden Hour',
            colorPalette: colorGrading || 'Teal and Orange Cinematic',
            aspectRatio: '16:9',
            confidence: 'high',
          },
        });
      ...
      const ai = new GoogleGenAI({ apiKey });
      const promptInstruction = `You are an expert AI Image Prompt Engineer.
The user provides a 1-line idea or keywords, along with specific stylistic parameters:
- Idea/Keywords: "${idea}"
- Lighting: "${lighting || 'Cinematic Golden Hour'}"
- Colour Grading: "${colorGrading || 'Teal and Orange Cinematic'}"
- Subject/Gender: "${gender || 'Any / Neutral'}"

Craft an extremely detailed, professional, high-converting AI image prompt (optimized for Midjourney, ChatGPT, Flux, and Gemini) incorporating these exact parameters, along with camera optics, textures, atmosphere, composition, and negative prompt.
Return the response strictly as JSON with keys: title, summary, prompt, negative_prompt, aspect_ratio, confidence.`;
      ...
      const jsonSchemaConfig = {
        ...
        responseSchema: {
          ...
          required: ['title', 'summary', 'prompt', 'negative_prompt', 'aspect_ratio', 'confidence'],
        },
      };

      try {
        const { response } = await generateWithModel(ai, selectedModel, {
          contents: promptInstruction,
          config: jsonSchemaConfig,
        });
        const parsed = JSON.parse(response.text || '{}');
        const normalizedData = {
          title: parsed.title || idea.slice(0, 40),
          summary: parsed.summary || `Detailed prompt for ${idea}`,
          promptText: parsed.prompt || '',
          negativePrompt: parsed.negative_prompt || 'blurry, low quality',
          aspectRatio: parsed.aspect_ratio || '16:9',
          lighting: lighting,
          colorPalette: colorGrading,
          confidence: parsed.confidence || 'high',
        };
        return NextResponse.json({ success: true, data: normalizedData });
      } catch (err: any) {
        const fallbackPrompt = `Photorealistic cinematic shot of ${idea}, featuring ${lighting} and ${colorGrading}, subject: ${gender}, highly detailed photography --ar 16:9 --v 6.1`;
        return NextResponse.json({
          success: true,
          data: {
            title: idea.slice(0, 40),
