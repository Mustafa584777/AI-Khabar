...
2. Provide structured JSON output with title, slug, category, promptText, negativePrompt, suggestedParameters, tags array (5-8 unique tags), seo, and articleContent.`;
    ...
    if (action === 'homepage_prompt_generator') {
      const selectedRatio = body.aspectRatio || '9:16';
      const userIdea = (body.topic || body.idea || '').trim() || 'Cinematic aesthetic portrait';

      const fallbackHomepagePrompt = {
        title: userIdea,
        promptText: `Hyper-realistic editorial photograph of ${userIdea}, captured on 35mm Hasselblad X2D 100C, 85mm f/1.2 lens, atmospheric natural rim lighting, delicate skin textures, lifelike depth of field, 8K ultra detail, photorealistic color grading --ar ${selectedRatio} --v 6.1 --style raw`,
        negativePrompt: 'blurry, oversaturated, deformed anatomy, extra fingers, cartoon, 3d render, plastic skin, watermark, low quality, artifacts',
        aspectRatio: selectedRatio,
        camera: 'Hasselblad X2D 100C with 85mm f/1.2 lens',
        lighting: 'Atmospheric natural rim lighting with soft golden bounce',
        tags: ['Photorealistic', 'Editorial', 'Gemini AI', selectedRatio, '8K Detail'],
      };

      if (!apiKey) {
        return NextResponse.json({
          success: true,
          data: fallbackHomepagePrompt,
          fallback: true,
        });
      }
      ...
      try {
        ...
        const systemInstruction = `You are a master AI prompt engineer specializing in photo-realistic visual generation for Midjourney v6.1, Flux.1, Stable Diffusion XL, and Gemini.
Generate a complete, breathtaking, production-grade photographic prompt based on the user's idea and selected aspect ratio.
Include specific camera body, lens focal length, aperture, lighting setup, textures, and the aspect ratio parameter (--ar ${selectedRatio}).
Respond strictly in JSON format.`;

        const contentsPayload = `User Idea / Concept: "${userIdea}"
Target Aspect Ratio: ${selectedRatio}

Create a photography prompt that produces an ultra-realistic, award-winning studio or cinematic shot.
Return JSON with:
- title: concise title (3-6 words)
- promptText: complete prompt ending with "--ar ${selectedRatio} --v 6.1 --style raw"
- negativePrompt: comprehensive list of negative tokens
- aspectRatio: "${selectedRatio}"
- camera: specific real-world camera and lens setup
- lighting: professional lighting description
- tags: array of 4-6 relevant aesthetic tags`;
        ...
        const { response, modelUsed } = await generateWithModelFallback(ai, {
          ...
          config: {
            ...
            responseSchema: {
              ...
              required: ['title', 'promptText', 'negativePrompt', 'aspectRatio', 'camera', 'lighting', 'tags'],
            },
          },
        });
        ...
        console.warn('Gemini generation failed, returning fallback:', err?.message);
        return NextResponse.json({
          success: true,
          data: fallbackHomepagePrompt,
          fallback: true,
